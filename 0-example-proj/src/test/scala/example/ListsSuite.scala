...
// bdmendes: Tests fixed alongside the file, ignore the comments.
...
class ListsSuite extends munit.FunSuite {
  ...
  test("sum with negative and repeated elements") {
    assertEquals(sum(List(1,1-1)), 1)
  }
  ...
  test("max with negative and repeated elements") {
    assertEquals(max(List(1,1,-1)), 1)
  }
