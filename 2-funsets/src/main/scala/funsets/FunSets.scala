...
trait FunSets extends FunSetsInterface {
  ...
  /**
   * Returns the empty set (bdmendes: custom function).
   */
  def emptySet(): FunSet = (_) => false
  ...
  def map(s: FunSet, f: Int => Int): FunSet = {
    @tailrec
    def iter(a: Int, curr_set: FunSet): FunSet = {
      if (a > bound) curr_set
      else if (s(a)) iter(a + 1, union(curr_set, singletonSet(f(a))))
      else iter(a + 1, curr_set)
    }
    iter(-bound, emptySet())
  }
