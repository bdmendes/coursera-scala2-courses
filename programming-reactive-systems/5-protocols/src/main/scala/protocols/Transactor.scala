...
object Transactor {
  ...
  private def idle[T](value: T, sessionTimeout: FiniteDuration): Behavior[PrivateCommand[T]] = {
    Behaviors.receive {
      case (ctx, Begin(replyTo)) =>
        println("spawned")
        val childHandler = ctx.spawnAnonymous(sessionHandler(value, ctx.self, Set.empty))
        ctx.watchWith(childHandler, RolledBack(childHandler))
        ctx.scheduleOnce(sessionTimeout, ctx.self, RolledBack(childHandler))
        replyTo ! childHandler // The client expects a new actor to interact with.
        inSession(value, sessionTimeout, childHandler)
      case _ =>
        Behaviors.same
    }
  }
  ...
  private def inSession[T](rollbackValue: T, sessionTimeout: FiniteDuration, sessionRef: ActorRef[Session[T]]): Behavior[PrivateCommand[T]] =
    Behaviors.receive {
      case (_, Begin(_)) =>
        Behaviors.unhandled
      case (ctx, RolledBack(sessionRef)) =>
        ctx.stop(sessionRef)
        idle(rollbackValue, sessionTimeout)
      case (ctx, Committed(sessionRef, value)) =>
        ctx.stop(sessionRef)
  ...
  /**
    ...
    * @param inSession Parent actor reference, to send the [[Committed]] message to
    * @param done Set of already applied [[Modify]] messages
    */
  private def sessionHandler[T](currentValue: T, inSession: ActorRef[Committed[T]], done: Set[Long]): Behavior[Session[T]] =
    Behaviors.receive {
      ...
      case (_, Modify(f, id, reply, replyTo)) =>
        ...
        if (done.contains(id)) {
          ...
          sessionHandler(f(currentValue), inSession, done + id)
        }
      case (ctx, Commit(reply, replyTo)) =>
        inSession ! Committed(ctx.self, currentValue)
        replyTo ! reply
        Behaviors.stopped
      case (_, Rollback()) =>
        // Parent will watch this actor and act accordingly.
        Behaviors.stopped
    }
}
