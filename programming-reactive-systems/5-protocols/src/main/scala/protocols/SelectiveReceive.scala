...
object SelectiveReceive {
  ...
  def apply[T: ClassTag](bufferCapacity: Int, initialBehavior: Behavior[T]): Behavior[T] = {
    val validatedBehaviour = Behavior.validateAsInitial(initialBehavior)
    Behaviors.withStash(bufferCapacity) (buf => intercept(bufferCapacity, buf, validatedBehaviour))
  }
  ...
  private def intercept[T: ClassTag](bufferSize: Int, buffer: StashBuffer[T], started: Behavior[T]): Behavior[T] =
    Behaviors.receive { case (ctx, message) =>
      ...
      // interceptor.
      val nextBehaviour = Behavior.interpretMessage(started, ctx, message)
      if (Behavior.isUnhandled(nextBehaviour)) {
        intercept(bufferSize, buffer.stash(message), started)
      } else {
        buffer.unstashAll(SelectiveReceive(bufferSize, Behavior.canonicalize(nextBehaviour, started, ctx)))
      }
    }
}
