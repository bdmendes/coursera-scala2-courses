...
object Server extends ServerModuleInterface {
  ...
  val eventParserFlow: Flow[ByteString, Event, NotUsed] =
    reframedFlow.via(Flow.fromFunction(Event.parse))
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    Flow[Event].statefulMapConcat { () =>
      ...
      var bufferedEvents: Vector[Event] = Vector.empty

      {
        event =>
          if (event.sequenceNr == expectedSequenceNumber) {
            var l = event :: Nil
            breakable {
              while (true) {
                expectedSequenceNumber += 1
                val next = bufferedEvents.find(_.sequenceNr == expectedSequenceNumber)
                if (next.isDefined) {
                  l = l.appended(next.get)
                  bufferedEvents = bufferedEvents.filterNot(_ == next.get)
                } else {
                  break
                }
              }
            ...
            bufferedEvents = bufferedEvents.appended(event)
            Nil
          }
      }
    }
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] =
    Flow[Event]
      ...
      .statefulMapConcat { () => {
          var followers: Followers = Map[Int, Set[Int]]().withDefaultValue(Set.empty)

          { e =>
            e match {
              case Event.Follow(_, from, to) =>
                followers = followers.updated(from, followers(from) + to)
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean = {
    ...
    event match {
      ...
      case StatusUpdate(_, fromUserId) => followers.getOrElse(userId, Set.empty).contains(fromUserId)
    }
  }
  ...
  def clientFlow(): Flow[ByteString, ByteString, NotUsed] = {
    ...
    val incoming: Sink[ByteString, NotUsed] =
      identityParserSink.mapMaterializedValue { identity =>
        identity.onComplete(clientIdPromise.complete)
        NotUsed
      }
